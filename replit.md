...
- **Multi-User Role-Based Access Control (RBAC)**: Implemented dual-role system (Master Admin and Admin User). Master Admin manages all restaurants and users. Admin users are restricted to managing only their assigned restaurant's menu.
- **User Management UI**: Added interface for Master Admin to create and assign Admin users to restaurants.
- **Backend Security**: Enforced role-based validation on all management routes.
